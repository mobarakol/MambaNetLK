"""
    Mamba3D for Point Cloud Processing v3
    
    Enhanced version with SE (Squeeze-and-Excitation) attention mechanism.
    Combines Mamba architecture with channel attention for improved feature learning.
"""
...
class SE_Layer(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        # Excitation operation: bottleneck structure composed of two-layer MLP
    ...
    def forward(self, x):
        ...
        return x * y.expand_as(x)        # Broadcast weights and apply to original feature map
...
def mlp_layers(nch_input, nch_layers, b_shared=True, bn_momentum=0.1, dropout=0.0):
    ...
    for i, outp in enumerate(nch_layers):
        ...
        if b_shared: # Only add SE module for shared-weight convolutional layers
...
class Mamba3DBlock(nn.Module):
    """Mamba3D block with SE attention mechanism"""
...
class Mamba3D_features(torch.nn.Module):
    """
    Mamba3D feature extractor v3
    Enhanced with SE attention mechanism
    """
